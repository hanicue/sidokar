...
if(isset($_POST['submit'])){
    ...
    $lama = $db->escapeString($_POST['lama']);
    $baru = $db->escapeString($_POST['password']);
    $konfirm = $db->escapeString($_POST['confirmpassword']);
$user_id = $_SESSION['user_id'];
    if(isset($_POST['lama']) && isset($_POST['password']) && isset($_POST['confirmpassword'])){

        $db->sql("SELECT * FROM user WHERE id_user=".$user_id);
        $cekUser = $db->getResult();
            foreach ($cekUser as $getData) {
                $cocok = $getData['password'];
            }
...
echo '<div class="alert alert-danger d-flex align-items-center" role="alert">
                                <div class="flex-00-auto">
                                    <i class="fa fa-fw fa-warning"></i> Password baru tidak sama!
                                </div>
                            </div>';
        }else{

        $db->update('user',array('password'=>$baru),'id_user='.$user_id);
        echo '<div class="alert alert-success d-flex align-items-center" role="alert">
                                <div class="flex-00-auto">
                                    <i class="fa fa-fw fa-check"></i> Berhasil diubah!, Anda harus masuk lagi
                                </div>
                            </div>';
                              echo '<script>var delayInMilliseconds=3000; setTimeout(function(){window.location.href=\'logout.php\';}, delayInMilliseconds);</script>';
                          }
    
        }else{
           echo '<div class="alert alert-warning d-flex align-items-center" role="alert">
                                <div class="flex-00-auto">
                                    <i class="fa fa-fw fa-warning"></i> Password lama salah!
                                </div>
                            </div>';
        }
    }
